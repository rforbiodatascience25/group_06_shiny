...
gene_dna <- function(length,
                     base_probs = c(A_prob = 0.25,
                                    T_prob = 0.25,
                                    C_prob = 0.25,
                                    G_prob = 0.25))
{
  ...
  # Error handling: Make sure the input sums to 1
  if(abs(sum(base_probs)) != 1){
    stop("The argument to the parameter 'base_probs' has to sum to 1")
  }
  
  # Sampling bases to create a dna vector
  dna_vector <- sample(
    x = c("A", "T", "C", "G"),
    size = length,
    replace = TRUE,
  ...
  # Paste the vector content into a string
  dna_string <- paste0(
    x = dna_vector,
    collapse = "")
  return(dna_string)
